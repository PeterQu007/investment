...
class phRETS {
   ...
   */
	...
	private $disable_encoding_fix = true;
	...
	public function SearchQuery($resource, $class, $query = "", $optional_params = array()) {
		...
		$search_arguments['Count'] = (!isset($optional_params['Count']) || empty($optional_params['Count'])) ? 1 : $optional_params['Count'];
		$search_arguments['Format'] = (!isset($optional_params['Format']) || empty($optional_params['Format'])) ? "COMPACT-DECODED" : $optional_params['Format'];
		$search_arguments['Limit'] = (!isset($optional_params['Limit']) || empty($optional_params['Limit'])) ? 99999999 : $optional_params['Limit'];
		...
		while ($continue_searching) {
			...
			$count = ($xml->{"RETS-RESPONSE"}->Pagination && $xml->{"RETS-RESPONSE"}->Pagination->TotalRecords)
						? $xml->{"RETS-RESPONSE"}->Pagination->TotalRecords
						: null;
			...
			return array("Count" => $count, "Properties" => $properties);
		}
	}
	...
	public function Disconnect() {
		...
		if ($this->debug_mode == true && $this->debug_log) {
	...
	public function Connect($login_url, $username, $password, $ua_pwd = "", $ua_name = "BarcodeRealty/1.0") {
		...
		if (empty($this->cookie_file)) {
			$this->cookie_file = tempnam("cookies", "phrets");
		}
		...
		if ($this->debug_mode == true) {
			...
			if(empty($this->debug_file))
				$this->debug_file = $_SERVER["DOCUMENT_ROOT"]."/rets_debug.txt";
			
			// open file handler to be used by cURL debug log
			$this->debug_log = fopen($this->debug_file, 'w');
			if(!$this->debug_log)
				echo "<br /><strong>Debug:</strong> Unable to open '".$this->debug_file."'...<br />";
			else
				echo "<br /><strong>Debug:</strong> Opened file '".$this->debug_file."' as #".$this->debug_log."<br />";
	...
	public function ParseXMLResponse($data = "")
	{
		...
		if (!empty($data))
		{
			...
			$data = str_replace("xmlns=\"urn:CREA.Search.Property\"", "xmlns=\"\"", $data);
	...
	public function RETSRequest($action, $parameters = "") {
		...
		if ($this->debug_mode == true) {
			...
			$_fwrite_success = @fwrite($this->debug_log, "\n\nNEW SEARCH:\n");
			$_fwrite_success = @fwrite($this->debug_log, $request_url ."\n");
			$_fwrite_success = @fwrite($this->debug_log, $response_body ."\n");
			if(!$_fwrite_success) {
				echo "<br /><strong>Debug:</strong> Unable to write debug to '".$this->debug_file."'...<br />";
			}
		}
	...
	private function read_custom_curl_headers($handle, $call_string) {
		...
		elseif(isset($this->last_response_headers[$last_remembered_header])) {
